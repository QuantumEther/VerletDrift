  ...
  if (speed >= minSpeed) {
    ...
    for (const name of wheelNames) {
      ...
      for (let s = 0; s < count; s++) {
        ...
        if (isLocked) {
          p.r = 0.55 + physicsRandom() * 0.08;
          p.g = 0.55 + physicsRandom() * 0.08;
          p.b = 0.57 + physicsRandom() * 0.08;
          p.alpha = (params.smokeOpacityLocked ?? 0.75) + physicsRandom() * 0.15;
        } else {
          p.r = 0.78 + physicsRandom() * 0.12;
          p.g = 0.78 + physicsRandom() * 0.12;
          p.b = 0.80 + physicsRandom() * 0.10;
          p.alpha = (params.smokeOpacityOverspun ?? 0.65) + physicsRandom() * 0.15;
        }
